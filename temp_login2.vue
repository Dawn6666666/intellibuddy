...
// 琛ㄥ崟寮曠敤鍜屾暟鎹?const loginFormRef = ref<FormInstance>()
...
const loginRules = reactive<FormRules>({
  email: [{required: true, message: '璇疯緭鍏ラ偖绠?}, {type: 'email', message: '閭鏍煎紡涓嶆纭?}],
  password: [{required: true, message: '璇疯緭鍏ュ瘑鐮?}]
})
const registerRules = reactive<FormRules>({
  username: [{required: true, message: '璇疯緭鍏ョ敤鎴峰悕'}],
  email: [{required: true, message: '璇疯緭鍏ラ偖绠?}, {type: 'email', message: '閭鏍煎紡涓嶆纭?}],
  password: [{required: true, message: '璇疯緭鍏ュ瘑鐮?}, {min: 6, message: '瀵嗙爜鑷冲皯6浣?}]
})
  ...
  await formEl.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.success('娉ㄥ唽鎴愬姛锛屽凡鑷姩鐧诲綍锛?)
        await router.push('/app')
      } catch (error) {
        ElMessage.error(error as string)
      } finally {
      ...
      <div class="container" :class="{ 'active': isRegisterActive }">
        <div class="form-container sign-up">
          <el-form :model="registerForm" :rules="registerRules" ref="registerFormRef"
            ...
            <span>鎴栦娇鐢ㄩ偖绠辨敞鍐?/span>
            <el-form-item prop="username">
              <el-input v-model="registerForm.username" placeholder="鐢ㄦ埛鍚?/>
            </el-form-item>
            <el-form-item prop="email">
              <el-input v-model="registerForm.email" type="email" placeholder="閭"/>
            </el-form-item>
            <el-form-item prop="password">
              <el-input v-model="registerForm.password" type="password" placeholder="瀵嗙爜" show-password/>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" native-type="submit" :loading="isLoading">娉ㄥ唽</el-button>
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="form-container sign-in">
          <el-form :model="loginForm" :rules="loginRules" ref="loginFormRef"
            ...
            <span>鎴栦娇鐢ㄩ偖绠卞瘑鐮佺櫥褰?/span>
            <el-form-item prop="email">
              <el-input v-model="loginForm.email" type="email" placeholder="閭"/>
            </el-form-item>
            <el-form-item prop="password">
              <el-input v-model="loginForm.password" type="password" placeholder="瀵嗙爜" show-password/>
            </el-form-item>
            <a href="#">蹇樿瀵嗙爜锛?/a>
            <el-form-item>
              <el-button type="primary" native-type="submit" :loading="isLoading">鐧诲綍</el-button>
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="toggle-container">
          <div class="toggle">
            <div class="toggle-panel toggle-left">
              <h1>娆㈣繋鍥炴潵锛?/h1>
              <p>杈撳叆鎮ㄧ殑涓汉淇℃伅浠ヤ娇鐢ㄦ墍鏈夌綉绔欏姛鑳?/p>
              <button class="hidden" @click="isRegisterActive = false">鐧诲綍</button>
            </div>
            <div class="toggle-panel toggle-right">
              <h1>浣犲ソ锛屾湅鍙嬶紒</h1>
              <p>娉ㄥ唽骞舵彁渚涙偍鐨勪釜浜轰俊鎭紝寮€濮嬫垜浠殑鏃呯▼鍚?/p>
              <button class="hidden" @click="isRegisterActive = true">娉ㄥ唽</button>
            </div>
          </div>
        </div>
      </div>
